...
def recommendMix(username):
    recommendations = []
    userData = userCollection.find_one({"username": username})

    totalTime = 0
    for h in userData['history']:
        totalTime = totalTime + h['time']

    for h in userData['history']:
        currAppid = h['appid']
        currPer = int((h['time']*100)/totalTime)
        currRec = recommendOnAppid(currAppid, currPer)
        for i in currRec:
